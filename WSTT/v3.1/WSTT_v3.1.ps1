...
# Trojan-overlap analysis and dynamic RPC range (49152-65535) are intentionally not live-scanned.
...
$script:WSFC_REQUIRED_PORTS = @(
    ...
    @{ Service = 'Cluster Service'; Protocol = 'ICMPv4'; Port = $null; Direction = 'Bidirectional'; Purpose = 'Add Node Wizard / validation ICMPv4 Echo.' },
    @{ Service = 'Cluster Service'; Protocol = 'ICMPv6'; Port = $null; Direction = 'Bidirectional'; Purpose = 'Cluster validation ICMPv6 Echo.' },
...
$script:WSFC_DYNAMIC_RPC_PORT_RANGE = '49152-65535'

# Optional dependency ports for file-share witness, SOFS, and general SMB paths.
# These are not all core Cluster Service ports. Use this profile when the cluster
# workload depends on SMB/file sharing or legacy NetBIOS over TCP/IP.
$script:WSFC_SMB_DEPENDENCY_PORTS = @(
    @{ Service = 'Microsoft File Sharing (RPC Endpoint Mapper)'; Protocol = 'TCP'; Port = 135; Direction = 'Bidirectional'; Purpose = 'RPC endpoint mapper for file-sharing/admin dependency paths.' },
    @{ Service = 'NetBIOS Name Resolution'; Protocol = 'UDP'; Port = 137; Direction = 'Bidirectional'; Purpose = 'Legacy NetBIOS name lookup when SMB over NetBIOS or app dependency requires it.' },
    @{ Service = 'NetBIOS Datagram Service'; Protocol = 'UDP'; Port = 138; Direction = 'Bidirectional'; Purpose = 'Legacy SMB over NetBIOS datagram traffic.' },
    @{ Service = 'NetBIOS Session Service'; Protocol = 'TCP'; Port = 139; Direction = 'Bidirectional'; Purpose = 'Legacy SMB over NetBIOS session traffic.' },
    @{ Service = 'Direct-hosted SMB'; Protocol = 'TCP'; Port = 445; Direction = 'Bidirectional'; Purpose = 'Modern SMB for file-share witness, SOFS, and general SMB access.' }
)
...
function Test-WSFCPortReachability {
    ...
    try {
        switch ($PortDefinition.Protocol) {
            'TCP' {
                ...
                try {
                    ...
                    elseif (-not $completed) {
                        $result.Status = 'Fail'
                        $result.ErrorMessage = "TCP connect timed out after ${TimeoutMs}ms (no response / possible drop)"
                    }
                    else {
                        $result.Status = 'Fail'
                        $result.ErrorMessage = 'TCP connect failed before a session was established'
                    }
                }
                catch [System.AggregateException] {
                    $socketError = $_.Exception.Flatten().InnerExceptions |
                        Where-Object { $_ -is [System.Net.Sockets.SocketException] } |
                        Select-Object -First 1
                    $result.Status = 'Fail'
                    if ($socketError) {
                        switch ($socketError.SocketErrorCode) {
                            'ConnectionRefused' { $result.ErrorMessage = 'TCP connection refused/reset by target (listener absent or rejecting)' }
                            'TimedOut'          { $result.ErrorMessage = "TCP connect timed out after ${TimeoutMs}ms (no response / possible drop)" }
                            default             { $result.ErrorMessage = "TCP socket error: $($socketError.SocketErrorCode) - $($socketError.Message)" }
                        }
                    }
                    else {
                        $result.ErrorMessage = "TCP connect failed: $($_.Exception.Message)"
                    }
                }
                    ...
                    switch ($_.Exception.SocketErrorCode) {
                        'ConnectionRefused' { $result.ErrorMessage = 'TCP connection refused/reset by target (listener absent or rejecting)' }
                        'TimedOut'          { $result.ErrorMessage = "TCP connect timed out after ${TimeoutMs}ms (no response / possible drop)" }
                        default             { $result.ErrorMessage = "TCP socket error: $($_.Exception.SocketErrorCode) - $($_.Exception.Message)" }
                    }
                }
                finally {
                    if ($tcpClient) {
            ...
            'ICMPv4' {
                try {
                    $pingOutput = & ping.exe -4 -n 2 -w $TimeoutMs $TargetNode 2>&1
                    if ($LASTEXITCODE -eq 0) {
                        $result.Status = 'Pass'
                    }
                    else {
                        $result.Status = 'Fail'
                        $result.ErrorMessage = "ICMPv4 Echo failed: $(($pingOutput | Select-Object -Last 1) -join '')"
                    }
                }
                catch {
                    $result.Status = 'Fail'
                    $result.ErrorMessage = "ICMPv4 test threw: $($_.Exception.Message)"
                }
            }
            'ICMPv6' {
                if ($TargetNode -match '^(\d{1,3}\.){3}\d{1,3}$') {
                    $result.Status = 'Inconclusive'
                    $result.ErrorMessage = 'ICMPv6 skipped for IPv4 literal target; use a hostname or IPv6 target to test IPv6 Echo.'
                }
                else {
                    try {
                        $pingOutput = & ping.exe -6 -n 2 -w $TimeoutMs $TargetNode 2>&1
                        if ($LASTEXITCODE -eq 0) {
                            $result.Status = 'Pass'
                        }
                        else {
                            $result.Status = 'Fail'
                            $result.ErrorMessage = "ICMPv6 Echo failed: $(($pingOutput | Select-Object -Last 1) -join '')"
                        }
                    }
                    catch {
                        $result.Status = 'Fail'
                        $result.ErrorMessage = "ICMPv6 test threw: $($_.Exception.Message)"
                    }
                }
            }
            default {
...
function Get-WSFCFirewallRuleStatus {
    ...
    try {
        ...
        $matchingFilters = switch ($PortDefinition.Protocol) {
            ...
            'ICMPv6' {
                Get-NetFirewallPortFilter -ErrorAction Stop |
                    Where-Object { $_.Protocol -eq 'ICMPv6' -and ($_.IcmpType -contains '128' -or $_.IcmpType -contains 'Any' -or $_.IcmpType -eq '128') }
            }
            default {
        ...
        $broadAllow    = @($matchingFilters | Where-Object { $_.LocalPort -eq 'Any' -or $_.IcmpType -contains 'Any' -or $_.IcmpType -eq 'Any' })
        ...
        if ($broadAllow.Count -gt 0) {
            $result.AuditNote = 'Matched broad firewall rule (LocalPort/IcmpType Any); verify this is intended and least-privilege.'
        }
    }
    catch {
...
function Start-WSFCPktmonCapture {
    <#
    .SYNOPSIS
        Starts a short pktmon capture for WSFC connectivity probes.
    .DESCRIPTION
        Uses built-in pktmon.exe when available. Capture is intentionally short:
        it starts immediately before WSFC reachability probes and is stopped
        immediately afterwards by Stop-WSFCPktmonCapture.
    #>
    [CmdletBinding()]
    param()
    ...
    if (-not $pktmon) {
        $capture.ErrorMessage = 'pktmon.exe not found on this host'
        Write-DiagWarning "  pktmon capture skipped - $($capture.ErrorMessage)."
        return $capture
    }
    ...
    if (-not $script:DefaultLogPath -or -not (Test-Path $script:DefaultLogPath)) {
        if (-not (Test-PathValid -Path $script:DefaultLogPath -CreateIfNotExist)) {
            $capture.ErrorMessage = "Could not initialise log path: $script:DefaultLogPath"
            Write-DiagWarning "  pktmon capture skipped - $($capture.ErrorMessage)."
            return $capture
        }
    }
    ...
    $capture.PktmonPath = $pktmon.Source
    $capture.EtlPath = Join-Path $script:DefaultLogPath "WSFC_Pktmon_$timestamp.etl"
    $capture.TextPath = Join-Path $script:DefaultLogPath "WSFC_Pktmon_$timestamp.txt"

    try {
        Write-Info "  Starting pktmon capture: $($capture.EtlPath)"
        Write-Info "  Note: pktmon capture is broad and short-lived; correlate timestamps with WSFC probe rows."
        $startArgs = @('start', '--capture', '--pkt-size', '0', '--file-name', $capture.EtlPath)
        $startOutput = & $capture.PktmonPath @startArgs 2>&1
        $startExitCode = $LASTEXITCODE
        if ($startExitCode -eq 0) {
            $capture.Started = $true
            Write-Success "  pktmon capture started"
        }
        else {
            $capture.ErrorMessage = ($startOutput | Out-String).Trim()
            Write-DiagWarning "  pktmon capture did not start (exit $startExitCode): $($capture.ErrorMessage)"
        }
    }
    catch {
        $capture.ErrorMessage = $_.Exception.Message
        Write-DiagWarning "  pktmon capture did not start: $($capture.ErrorMessage)"
    }

    return $capture
}

function Stop-WSFCPktmonCapture {
    <#
    .SYNOPSIS
        Stops pktmon and formats the ETL capture when possible.
    #>
    [CmdletBinding()]
    param(
    ...
    if (-not $Capture.Started -or -not $Capture.PktmonPath) { return }

    try {
        $stopOutput = & $Capture.PktmonPath stop 2>&1
        $stopExitCode = $LASTEXITCODE
        if ($stopExitCode -eq 0) {
            Write-Success "  pktmon capture stopped"
        }
        else {
            Write-DiagWarning "  pktmon stop returned exit ${stopExitCode}: $(($stopOutput | Out-String).Trim())"
        }
    }
    catch {
        Write-DiagWarning "  pktmon stop failed: $($_.Exception.Message)"
    }

    if ($Capture.EtlPath -and (Test-Path $Capture.EtlPath)) {
        Write-Success "  pktmon ETL: $($Capture.EtlPath)"
        try {
            $formatOutput = & $Capture.PktmonPath etl2txt $Capture.EtlPath --out $Capture.TextPath 2>&1
            $formatExitCode = $LASTEXITCODE
            if ($formatExitCode -eq 0 -and (Test-Path $Capture.TextPath)) {
                Write-Success "  pktmon text: $($Capture.TextPath)"
                Show-WSFCPktmonReviewSummary -Path $Capture.TextPath
            }
            else {
                Write-DiagWarning "  pktmon format returned exit ${formatExitCode}: $(($formatOutput | Out-String).Trim())"
            }
        }
        catch {
            Write-DiagWarning "  pktmon format failed: $($_.Exception.Message)"
        }
    }
    else {
        Write-DiagWarning "  pktmon ETL file was not found after capture stop"
    }
}

function Show-WSFCPktmonReviewSummary {
    <#
    .SYNOPSIS
        Reviews pktmon text output and prints a concise WSFC connectivity summary.
    .DESCRIPTION
        Heuristic parser for pktmon etl2txt output captured during WSFC probes.
        It highlights DNS resolution, active remote refusals (RST / ICMP port
        unreachable), and SYN/SYN-ACK evidence for tested ports. It is meant as
        a triage summary, not a full packet-analysis replacement.
    .PARAMETER Path
        pktmon text output generated by pktmon etl2txt.
    #>
    [CmdletBinding()]
    param(
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        Write-DiagWarning "  pktmon review skipped - text file not found: $Path"
        return
    }

    Write-Section "Pktmon Review Summary"
    Write-Info "  Source: $Path"
    Write-Info "  Note: pktmon logs the same packet at multiple stack components; duplicate Appearance rows are expected."

    Write-DiagWarning '  pktmon text can include hostnames, IP addresses, and MAC addresses. Redact before sharing outside the approved support boundary.'

    try {
        $lines = Get-Content -LiteralPath $Path -ErrorAction Stop
    }
    catch {
        Write-DiagWarning "  Could not read pktmon text output: $($_.Exception.Message)"
        return
    }

    $packetRecords = [System.Collections.Generic.List[object]]::new()
    $currentPacketId = ''
    foreach ($line in $lines) {
        if ($line -match 'PktGroupId\s+(?<PacketId>\d+)') { $currentPacketId = $Matches.PacketId }
        if ($line -match '^\s*(?:[0-9A-Fa-f]{2}-|Drop:\s+ip:)') {
            $packetRecords.Add([pscustomobject]@{
    ...
    $packetLines = @($packetRecords | Select-Object -ExpandProperty Text -Unique)
    if ($packetRecords.Count -eq 0) {
        Write-DiagWarning "  No decoded packet lines found in pktmon text output."
        return
    }

    $dnsAnswers = @($packetLines | Where-Object { $_ -match '\.53\s*>\s*.+\s+A\s+\d{1,3}(?:\.\d{1,3}){3}' })
    $dnsQueriesById = @{}
    $targetNamesByIp = @{}

    foreach ($line in $packetLines) {
        if ($line -match ':\s*(?<QueryId>\d+)\+\s+A\?\s+(?<Name>[^\s]+)') {
            $dnsQueriesById[$Matches.QueryId] = $Matches.Name.TrimEnd('.')
        }
        elseif ($line -match ':\s*(?<QueryId>\d+)\*\s+.*\s+A\s+(?<Ip>\d{1,3}(?:\.\d{1,3}){3})') {
            if ($dnsQueriesById.ContainsKey($Matches.QueryId)) {
                $targetNamesByIp[$Matches.Ip] = $dnsQueriesById[$Matches.QueryId]
            }
        }
    }

    if ($dnsAnswers.Count -gt 0) {
        Write-Success "  DNS resolution evidence found ($($dnsAnswers.Count) decoded answer row(s))."
        foreach ($line in ($dnsAnswers | Select-Object -First 3)) {
            if ($line -match 'A\s+(?<ip>\d{1,3}(?:\.\d{1,3}){3})') {
                Write-Info "    Resolved target IP: $($Matches.ip)"
            }
        }
    }
    else {
        Write-DiagWarning "  No DNS A-answer evidence found in pktmon text. This may be normal if the target was already cached or an IP literal was used."
    }

    $udpUnreachable = @($packetLines | Where-Object { $_ -match 'udp port (?<port>\d+) unreachable' })
    $tcpReset = @($packetLines | Where-Object { $_ -match 'Flags \[R\.\].*\.(?<port>\d+):|(?<port2>\d+)\s*>\s*.+:\s+Flags \[R\.\]' })
    $synAck = @($packetLines | Where-Object { $_ -match 'Flags \[S\.E?\]|Flags \[S\.\]' })
    $synOnly = @($packetLines | Where-Object { $_ -match 'Flags \[S(EW)?\]' })

    $knownPorts = @($script:WSFC_REQUIRED_PORTS + $script:WSFC_SMB_DEPENDENCY_PORTS |
        Where-Object { $_ -is [hashtable] -and $null -ne $_['Port'] } |
        ForEach-Object { "$($_['Port'])" } |
        Select-Object -Unique)
    $outcomesByKey = @{}
    $statusRank = @{ Inconclusive = 1; Warning = 2; Fail = 3; Pass = 4 }

    function Add-PktmonOutcome {
        param(
            [string]$TargetIp,
            [string]$Protocol,
            [string]$Port,
        ...
        if ([string]::IsNullOrWhiteSpace($TargetIp) -or [string]::IsNullOrWhiteSpace($Protocol) -or [string]::IsNullOrWhiteSpace($Port)) { return }
        $targetName = if ($targetNamesByIp.ContainsKey($TargetIp)) { $targetNamesByIp[$TargetIp] } else { $TargetIp }
        $key = "$TargetIp|$Protocol|$Port"
        if (-not $outcomesByKey.ContainsKey($key)) {
            $outcomesByKey[$key] = [pscustomobject]@{
            ...
            if (-not [string]::IsNullOrWhiteSpace($PacketId)) { $outcomesByKey[$key].PacketIds[$PacketId] = $true }
            return
        }

        $row = $outcomesByKey[$key]
        if (-not [string]::IsNullOrWhiteSpace($PacketId)) {
            $row.PacketIds[$PacketId] = $true
            $row.Count = $row.PacketIds.Count
        }
        else {
            $row.Count++
        }
        if ($row.TargetName -eq $row.TargetIP -and $targetName -ne $TargetIp) { $row.TargetName = $targetName }
        if ($row.Evidence -notlike "*$Evidence*") { $row.Evidence = "$($row.Evidence); $Evidence" }
        if ($statusRank[$Status] -gt $statusRank[$row.Status]) {
            $row.Status = $Status
            $row.Action = $Action
        }
    }
    ...
    foreach ($record in $packetRecords) {
        ...
        if ($line -match 'ICMP (?<TargetIp>\d{1,3}(?:\.\d{1,3}){3}) udp port (?<Port>\d+) unreachable') {
            Add-PktmonOutcome -TargetIp $Matches.TargetIp -Protocol 'UDP' -Port $Matches.Port -Status 'Fail' -Evidence 'Remote ICMP port unreachable' -Action 'Target received the probe but reports the UDP port is unreachable; verify service role/listener and firewall on target.' -PacketId $packetId
            continue
        }

        if ($line -match '(?<SrcIp>\d{1,3}(?:\.\d{1,3}){3})\.(?<SrcPort>\d+)\s*>\s*(?<DstIp>\d{1,3}(?:\.\d{1,3}){3})\.(?<DstPort>\d+):\s+Flags \[(?<Flags>[^\]]+)\]') {
            $srcIp = $Matches.SrcIp
            ...
            if ($flags -match '^R\.?$') {
                if ($knownPorts -contains $srcPort) {
                    Add-PktmonOutcome -TargetIp $srcIp -Protocol 'TCP' -Port $srcPort -Status 'Fail' -Evidence 'RST from target' -Action 'Target actively refused the connection; verify listener/service state on target.' -PacketId $packetId
                }
                elseif ($knownPorts -contains $dstPort) {
                    Add-PktmonOutcome -TargetIp $dstIp -Protocol 'TCP' -Port $dstPort -Status 'Fail' -Evidence 'RST on tested port' -Action 'Connection was reset; verify listener/service state on target.' -PacketId $packetId
                }
                continue
            }

            if ($flags -match '^S\.E?$|^S\.$') {
                if ($knownPorts -contains $srcPort) {
                    Add-PktmonOutcome -TargetIp $srcIp -Protocol 'TCP' -Port $srcPort -Status 'Pass' -Evidence 'SYN-ACK from target' -Action 'TCP listener accepted the handshake.' -PacketId $packetId
                }
                continue
            }

            if ($flags -match '^S(EW)?$') {
                if ($knownPorts -contains $dstPort) {
                    Add-PktmonOutcome -TargetIp $dstIp -Protocol 'TCP' -Port $dstPort -Status 'Warning' -Evidence 'SYN sent' -Action 'If no SYN-ACK or RST follows, suspect firewall drop, routing loss, or packet loss.' -PacketId $packetId
                }
            }
        }
        elseif ($line -match '(?<TargetIp>\d{1,3}(?:\.\d{1,3}){3})\s*>\s*\d{1,3}(?:\.\d{1,3}){3}:\s+ICMP echo reply') {
            Add-PktmonOutcome -TargetIp $Matches.TargetIp -Protocol 'ICMPv4' -Port 'Echo' -Status 'Pass' -Evidence 'ICMP echo reply' -Action 'ICMPv4 path is reachable.' -PacketId $packetId
        }
    }

    if ($targetNamesByIp.Count -gt 0) {
        Write-Section "Pktmon Name Resolution Matrix"
        $dnsRows = foreach ($ip in ($targetNamesByIp.Keys | Sort-Object)) {
            [pscustomobject]@{
                TargetName = $targetNamesByIp[$ip]
                TargetIP   = $ip
                Status     = 'Pass'
                Evidence   = 'DNS A answer observed'
            }
        }
        $dnsRows | Format-Table TargetName, TargetIP, Status, Evidence -AutoSize | Out-String -Width 220 | ForEach-Object { Write-Host $_ }
    }

    if ($udpUnreachable.Count -gt 0) {
        Write-DiagError "  Remote UDP port-unreachable evidence found ($($udpUnreachable.Count) decoded row(s))."
        foreach ($group in ($udpUnreachable | ForEach-Object {
                    if ($_ -match 'udp port (?<port>\d+) unreachable') { $Matches.port }
                } | Group-Object | Sort-Object Name)) {
            Write-DiagWarning "    UDP/$($group.Name) unreachable x$($group.Count)"
        }
    }

    if ($tcpReset.Count -gt 0) {
        Write-DiagError "  TCP reset evidence found ($($tcpReset.Count) decoded row(s))."
        foreach ($group in ($tcpReset | ForEach-Object {
                    if ($_ -match '(?<SrcIp>\d{1,3}(?:\.\d{1,3}){3})\.(?<SrcPort>\d+)\s*>\s*(?<DstIp>\d{1,3}(?:\.\d{1,3}){3})\.(?<DstPort>\d+):\s+Flags \[R\.\]') {
                        if ($knownPorts -contains $Matches.SrcPort) { $Matches.SrcPort }
                        elseif ($knownPorts -contains $Matches.DstPort) { $Matches.DstPort }
                    }
                } | Where-Object { $_ } | Group-Object | Sort-Object Name)) {
            Write-DiagWarning "    TCP/$($group.Name) reset x$($group.Count)"
        }
    }

    if ($synAck.Count -gt 0) {
        Write-Success "  TCP SYN-ACK evidence found ($($synAck.Count) decoded row(s)); at least one TCP listener accepted a handshake."
    }
    elseif ($synOnly.Count -gt 0) {
        Write-DiagWarning "  SYN packets were observed but no SYN-ACK rows were decoded; check for timeout, firewall drop, or remote listener absence."
    }

    if ($udpUnreachable.Count -eq 0 -and $tcpReset.Count -eq 0 -and $synAck.Count -eq 0 -and $synOnly.Count -eq 0) {
        Write-Info "  No WSFC probe outcome patterns were detected. Review the pktmon text manually for target/port rows."
    }

    if ($outcomesByKey.Count -gt 0) {
        Write-Section "Pktmon Target / Port Outcome Matrix"
        $outcomeRows = @($outcomesByKey.Values | Sort-Object TargetName, TargetIP, Protocol, Port)
        $outcomeRows |
            Select-Object TargetName, TargetIP, Protocol, Port, Status, Evidence, Count, Action |
    ...
    Write-Info "    - DNS answer + RST/port-unreachable means the packet reached the target, but the target refused that port."
    Write-Info "    - SYN with no response usually points to firewall drop, routing loss, or packet loss."
    Write-Info "    - SYN/SYN-ACK means the TCP path and listener were reachable for that probe."
}
...
function Test-WSFCClusterPortCompliance {
    ...
    .PARAMETER CapturePktmon
        Capture a short built-in pktmon trace during live reachability probes.
    .PARAMETER PortDefinitions
        Internal/custom port definitions to test. Defaults to core WSFC Cluster Service ports.
    .PARAMETER CheckTitle
        Report title for custom port profiles.
    .PARAMETER RequirementSummary
        Short one-line summary of the port profile being tested.
    .EXAMPLE
    ...
    param(
        ...
        [switch]$ExportCsv,
        [switch]$ExportHtml,
        [switch]$CapturePktmon,
        [object[]]$PortDefinitions = $script:WSFC_REQUIRED_PORTS,
        [string]$CheckTitle = 'WSFC Cluster Port Compliance Check',
        [string]$RequirementSummary = 'Tested static requirements: UDP/TCP 3343, ICMPv4/v6 Echo, TCP 445, TCP 135, UDP 137, TCP 5985.'
    )

    Write-Header $CheckTitle
    ...
    if ($RequirementSummary) { Write-Info "  $RequirementSummary" }
    Write-Info "  Dynamic RPC range $script:WSFC_DYNAMIC_RPC_PORT_RANGE is documented but NOT live-scanned."
    Write-Info "  Trojan-port overlap analysis is NOT tested."
    Write-Host ""
    ...
    if (-not $SkipReachability) {
        $pktmonCapture = $null
        if ($CapturePktmon) { $pktmonCapture = Start-WSFCPktmonCapture }
        ...
        try {
            ...
            if ($pktmonCapture -and $pktmonCapture.Started) { Stop-WSFCPktmonCapture -Capture $pktmonCapture }
            }
    }
    else {
        Write-Info "  Reachability tests skipped."
        if ($CapturePktmon) { Write-DiagWarning "  pktmon capture skipped because reachability tests are disabled." }
    }
    ...
    if (-not $SkipFirewallAudit) {
        ...
        foreach ($portDef in $PortDefinitions) {
            ...
            if ($r.AuditNote) { Write-DiagWarning "      $($r.AuditNote)" }
            if ($r.ErrorMessage) { Write-Info "      $($r.ErrorMessage)" }
        }
    }
    else {
...
function Test-WSFCClusterBuildPortReadiness {
    <#
    .SYNOPSIS
        Checks WSFC ports before creating a cluster or adding nodes.
    .DESCRIPTION
        Runs the WSFC static-port reachability and local firewall audit using
        manually supplied target nodes. Use this before New-Cluster or
        Add-ClusterNode when auto-discovery is not available yet.
    .PARAMETER TargetNode
        Planned peer nodes or nodes being added to the cluster.
    .PARAMETER Scenario
        Whether this check is for a new cluster build or adding a node.
    .PARAMETER ExportCsv
        Write per-port CSV files under $script:DefaultLogPath.
    .PARAMETER ExportHtml
        Write a single dark-themed HTML report under $script:DefaultLogPath.
    .PARAMETER CapturePktmon
        Capture a short built-in pktmon trace during live reachability probes.
    .EXAMPLE
        Test-WSFCClusterBuildPortReadiness -TargetNode 'node02'
    .EXAMPLE
        Test-WSFCClusterBuildPortReadiness -TargetNode 'node02','node03' -ExportCsv -ExportHtml
    #>
    [CmdletBinding()]
    param(
        [string[]]$TargetNode,
        [ValidateSet('BuildCluster', 'AddNode')]
        [string]$Scenario = 'BuildCluster',
        [switch]$ExportCsv,
        [switch]$ExportHtml,
        [switch]$CapturePktmon
    )

    Write-Header "WSFC Build / Add-Node Port Readiness"
    if ($Scenario -eq 'BuildCluster') {
        Write-Info "  Scenario: Build a new failover cluster before New-Cluster."
    }
    else {
        Write-Info "  Scenario: Add a node to an existing failover cluster before Add-ClusterNode."
    }
    Write-Info "  This proves reachability from THIS node to supplied targets and audits THIS node's firewall rules."
    if ($Scenario -eq 'BuildCluster') {
        Write-Info "  For bidirectional evidence, run it from every planned node."
    }
    else {
        Write-Info "  For bidirectional evidence, run from an existing node to the new node, then from the new node back to existing nodes."
    }
    Write-Info "  Dynamic RPC range 49152-65535 is not live-scanned; static WSFC ports below are tested."
    Write-Host ""
    ...
    if ($TargetNode -and $TargetNode.Count -gt 0) {
        foreach ($node in $TargetNode) {
            $trimmed = $node.Trim()
            if ($trimmed -match $script:HOSTNAME_REGEX) { $targets.Add($trimmed) }
            else { Write-DiagWarning "  Skipping invalid target '$trimmed'" }
        }
    }

    if ($targets.Count -eq 0) {
        $manual = Read-Host "  Enter planned peer/new node names (comma-separated)"
        if (-not [string]::IsNullOrWhiteSpace($manual)) {
            foreach ($node in ($manual -split ',')) {
                $trimmed = $node.Trim()
                if ($trimmed -match $script:HOSTNAME_REGEX) { $targets.Add($trimmed) }
                else { Write-DiagWarning "  Skipping invalid target '$trimmed'" }
            }
    ...
    if ($targets.Count -eq 0) {
        Write-DiagError "  No valid target nodes supplied. Cannot run build/add-node readiness check."
        return
    }

    Write-Section "How to Use This Result"
    if ($Scenario -eq 'BuildCluster') {
        Write-Info "  New 2-node cluster: run from node01 with -TargetNode node02, then from node02 with -TargetNode node01."
        Write-Info "  New 3-node cluster: run from each node against the other two planned nodes."
        Write-Info "  After ports pass, run Microsoft's Test-Cluster validation before New-Cluster."
    }
    else {
        Write-Info "  Add node: run from an existing cluster node to the new node, then from the new node to existing nodes."
        Write-Info "  This check does not replace Add-ClusterNode validation or cluster health checks; it only proves required static ports."
    }
    Write-Host ""

    Test-WSFCClusterPortCompliance -TargetNode $targets.ToArray() -ExportCsv:$ExportCsv -ExportHtml:$ExportHtml -CapturePktmon:$CapturePktmon
}

function Test-WSFCFileSharePortReadiness {
    <#
    .SYNOPSIS
        Checks optional SMB/file-share dependency ports for WSFC scenarios.
    .DESCRIPTION
        Use this when validating file-share witness, SOFS, general SMB access,
        or legacy SMB over NetBIOS dependencies. These ports are separate from
        the core Cluster Service port matrix.
    .PARAMETER TargetNode
        Peer nodes or file-share/SOFS targets to test.
    .PARAMETER ExportCsv
        Write per-port CSV files under $script:DefaultLogPath.
    .PARAMETER ExportHtml
        Write a single dark-themed HTML report under $script:DefaultLogPath.
    ...
    Write-Header "WSFC File-Share / SMB Dependency Port Readiness"
    Write-Info "  Use when the cluster design includes file-share witness, SOFS, general SMB access, or legacy SMB over NetBIOS."
    Write-Info "  Modern SMB normally requires TCP 445. NetBIOS ports 137/138/139 are legacy/application-dependent."
    Write-Info "  This is an optional dependency profile, not the core Cluster Service port matrix."
    Write-Host ""
    ...
    Test-WSFCClusterPortCompliance `
        ...
        -CapturePktmon:$CapturePktmon `
        -SkipFirewallAudit:$SkipFirewallAudit `
        -PortDefinitions $script:WSFC_SMB_DEPENDENCY_PORTS `
        -CheckTitle 'WSFC File-Share / SMB Dependency Port Readiness' `
        -RequirementSummary 'Tested SMB dependency ports: TCP 135, UDP 137, UDP 138, TCP 139, TCP 445.'
}
...
function Show-WSFCPortSummaryTable {
    ...
    [CmdletBinding()]
    param(
        [System.Collections.Generic.List[object]]$ReachabilityResults,
        [System.Collections.Generic.List[object]]$FirewallResults,
    ...
    if ($FirewallResults.Count -gt 0) {
        ...
        $FirewallResults |
            ...
            Format-Table Service, Protocol, Port, InboundAllow, OutboundAllow, BlockingRules, AuditNote -AutoSize |
...
function Export-WSFCPortReportToHtml {
    ...
    $fwRowsHtml = if ($FirewallResults.Count -gt 0) {
        ($FirewallResults | Sort-Object Service, Protocol, Port | ForEach-Object {
            ...
            $notes = @(if ($_.AuditNote) { $_.AuditNote }; if ($_.ErrorMessage) { $_.ErrorMessage }) -join ' '
            "<tr><td>$(& $enc $_.Service)</td><td>$(& $enc $_.Protocol)</td><td>$(& $enc $_.Port)</td><td class='$inCls'>$(& $enc $_.InboundAllow) ($($_.InboundRuleCount))</td><td class='$outCls'>$(& $enc $_.OutboundAllow) ($($_.OutboundRuleCount))</td><td class='$blockCls'>$(& $enc $_.BlockingRules)</td><td>$(& $enc $notes)</td></tr>"
        }) -join "`n"
    ...
    try {
        do {
            ...
            switch ($choice) {
                ...
                "22" {
                    Clear-Host
                    Write-Header "WSFC Cluster Port Checks"
                    Write-Host "1. Existing cluster compliance (auto-discover cluster peers)" -ForegroundColor Yellow
                    Write-Host "2. Build new cluster readiness (manual planned nodes)" -ForegroundColor Yellow
                    Write-Host "3. Add node readiness (manual new/existing nodes)" -ForegroundColor Yellow
                    Write-Host "4. File-Share / SMB dependency readiness (witness, SOFS, legacy SMB)" -ForegroundColor Yellow
                    Write-Host "0. Return to Main Menu" -ForegroundColor Yellow

                    $wsfcMode = Get-ValidatedChoice -Prompt "`nEnter choice (0-4)" -ValidChoices @("0", "1", "2", "3", "4")
                    switch ($wsfcMode) {
                        "1" {
                            $captureChoice = Get-ValidatedChoice -Prompt "Capture pktmon trace during reachability probes? (Y/N)" -ValidChoices @("Y", "N")
                            $capturePktmon = ($captureChoice -eq "Y")
                            Test-WSFCClusterPortCompliance -CapturePktmon:$capturePktmon
                            Write-Host "`n"
                            $exportChoice = Get-ValidatedChoice -Prompt "Export WSFC port report (CSV + HTML)? (Y/N)" -ValidChoices @("Y", "N")
                            if ($exportChoice -eq "Y") {
                                Test-WSFCClusterPortCompliance -ExportCsv -ExportHtml -CapturePktmon:$capturePktmon
                            }
                        }
                        "2" {
                            $manualTargets = Read-Host "Enter planned peer node names (comma-separated)"
                            $targetNodes = @($manualTargets -split ',' | ForEach-Object { $_.Trim() } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
                            Write-Host "`n"
                            $captureChoice = Get-ValidatedChoice -Prompt "Capture pktmon trace during reachability probes? (Y/N)" -ValidChoices @("Y", "N")
                            $capturePktmon = ($captureChoice -eq "Y")
                            $exportChoice = Get-ValidatedChoice -Prompt "Export WSFC new-cluster readiness report (CSV + HTML)? (Y/N)" -ValidChoices @("Y", "N")
                            $exportReport = ($exportChoice -eq "Y")
                            Test-WSFCClusterBuildPortReadiness -TargetNode $targetNodes -Scenario BuildCluster -ExportCsv:$exportReport -ExportHtml:$exportReport -CapturePktmon:$capturePktmon
                        }
                        "3" {
                            $manualTargets = Read-Host "Enter new node or existing peer node names to test (comma-separated)"
                            $targetNodes = @($manualTargets -split ',' | ForEach-Object { $_.Trim() } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
                            Write-Host "`n"
                            $captureChoice = Get-ValidatedChoice -Prompt "Capture pktmon trace during reachability probes? (Y/N)" -ValidChoices @("Y", "N")
                            $capturePktmon = ($captureChoice -eq "Y")
                            $exportChoice = Get-ValidatedChoice -Prompt "Export WSFC add-node readiness report (CSV + HTML)? (Y/N)" -ValidChoices @("Y", "N")
                            $exportReport = ($exportChoice -eq "Y")
                            Test-WSFCClusterBuildPortReadiness -TargetNode $targetNodes -Scenario AddNode -ExportCsv:$exportReport -ExportHtml:$exportReport -CapturePktmon:$capturePktmon
                        }
                        "4" {
                            $manualTargets = Read-Host "Enter file-share witness/SOFS/SMB target names (comma-separated)"
                            $targetNodes = @($manualTargets -split ',' | ForEach-Object { $_.Trim() } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
                            Write-Host "`n"
                            $captureChoice = Get-ValidatedChoice -Prompt "Capture pktmon trace during SMB dependency probes? (Y/N)" -ValidChoices @("Y", "N")
                            $capturePktmon = ($captureChoice -eq "Y")
                            $exportChoice = Get-ValidatedChoice -Prompt "Export WSFC SMB dependency report (CSV + HTML)? (Y/N)" -ValidChoices @("Y", "N")
                            $exportReport = ($exportChoice -eq "Y")
                            Test-WSFCFileSharePortReadiness -TargetNode $targetNodes -ExportCsv:$exportReport -ExportHtml:$exportReport -CapturePktmon:$capturePktmon
                        }
                    }
                }
                "23" {
